...
import { ApiRoutesWithoutPrefix, apiUrl, HttpMethod } from '@/config';
import ApiServiceType, { apiService } from '@/services/apiService';
...
export const userApi = apiService.injectEndpoints({
   endpoints: (builder) => ({
      ...
      addUser: builder.mutation<User, Partial<UserRegistration>>({
         query: (data) => ({
      ...
      login: builder.mutation<UserResponse, LoginRequest>({
         query: (credentials) => {
            return {
               url: `${apiUrl}${ApiRoutesWithoutPrefix.LOGIN}`,
               method: HttpMethod.POST,
               body: credentials,
            };
         },
